...
%% ---- USER SETTINGS -------------------------------------------------------
expName      = 'Butterfly painting v2';
env          = 'indoor';
isRaw        = true;
rotateImages = true;
%defaultBands = [70,53,19];
defaultBands = [27,16,7]; % for 64-band
...
%% ---- LOAD CUBES ----------------------------------------------------------
IM = fetchExperimentData(expName, env, isRaw);
N = numel(IM);
datas   = cell(1,N);
imRGB   = cell(1,N);
sizesHW = zeros(N,2);
...
if ~isempty(defaultBands)
    visParams.idxNIR = defaultBands(1);
    visParams.idxRed = defaultBands(2);
    visParams.idxGreen = defaultBands(3);
end
...
for i = 1:N
    ...
    imRGB{i} = visualizeHsiFalseColor_SHARED(datas{i}, visParams, 1);
end
...
for i = 1:N
    ...
    title(ax(i), sprintf('%s', IM(i).imname), 'Interpreter','none');
    hold(ax(i), 'on');
...
function [Y, labels] = computeAllSpectra(datas, x, y, win, IM)
    ...
    for k = 1:Nloc
        ...
        labels{k} = sprintf('%s @ (%d,%d)', IM(k).imname, x, y);
    end
end
